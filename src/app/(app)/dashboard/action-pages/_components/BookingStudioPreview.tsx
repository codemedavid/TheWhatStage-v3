...
import BookingPicker from '@/app/a/[slug]/_kinds/booking/Renderer.client'
import type { BookingConfig } from '@/app/a/[slug]/_kinds/booking/schema'
...
/**
 * Studio-mode preview: a 1:1 clone of the public Booking renderer's
 * `standalone` variant, hooked to the studio's draft state so it updates
 * live as the operator edits.
 *
 * Kept as a parallel client component (rather than importing the server
 * renderer) because BookingStudioShell is a client component.
 */
export function BookingStudioPreview({
  title,
  description,
  slug,
  config,
  ...
  const safeTitle = title.trim() || 'Untitled booking'
    ...
    >
      ...
      <div className="flex min-h-0 flex-col gap-[18px] px-6 py-7 md:px-7">
        <BookingPicker
          slug={slug}
          config={config}
          hidden={{ slug, p: null, g: null, e: null, t: null }}
          sourceContext={null}
        />
      </div>
    </section>
  ...
  return (
    <main
      className="flex min-h-full flex-col bg-[#FAFAF7] text-[#1A1915]"
      style={cssVars}
    >
      <div className="flex items-center justify-between px-7 py-5">
        <span className="inline-flex items-center gap-2.5 text-[#1A1915]">
          ...
          >
            {initials(safeTitle).slice(0, 1)}
          </span>
          <span className="text-[13px] font-semibold">{safeTitle}</span>
        </span>
      ...
      <div className="flex flex-1 items-start justify-center px-5 pb-12 pt-3">
