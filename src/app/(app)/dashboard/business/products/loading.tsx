  ...
  return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="flex gap-2">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="h-8 w-16 rounded-full bg-[#EEF0F3]" />
        ))}
        <div className="ml-auto h-9 w-56 rounded-md bg-[#EEF0F3]" />
      </div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="rounded-xl border border-[#E5E7EB] bg-white p-4">
            <div className="h-40 rounded-md bg-[#F3F4F6]" />
            <div className="mt-3 h-4 w-40 rounded bg-[#EEF0F3]" />
            <div className="mt-2 h-3 w-24 rounded bg-[#F3F4F6]" />
            <div className="mt-3 flex items-center justify-between">
              <div className="h-5 w-16 rounded-full bg-[#F3F4F6]" />
              <div className="h-5 w-12 rounded bg-[#EEF0F3]" />
            </div>
          </div>
        ))}
      </div>
