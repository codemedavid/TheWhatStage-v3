import type { ProjectsQuery } from './schemas'
...
/**
 * Resolve a {@link ProjectsQuery}'s `range` preset into concrete `from`/`to`
 * day bounds (matched against each project's `updated_at`). Presets are anchored
 * to "now" so they always track the current day/week/month; `custom` passes the
 * explicit bounds through and `all` clears them. Returns a new params object
 * (never mutates the input). Mirrors the leads board's `resolveDateRange`.
 */
export function resolveProjectsDateRange(
