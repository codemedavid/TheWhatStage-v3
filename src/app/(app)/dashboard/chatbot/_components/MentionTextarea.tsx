...
export function MentionTextarea({
  ...
  const actionPageItems: MediaMentionItem[] = useMemo(
    () =>
      actionPages.map((p) => ({
        kind: 'action_page' as const,
        id: p.id,
        slug: p.slug,
        name: p.title,
        ctaLabel: p.ctaLabel,
  ...
  const matches = useMemo<MediaMentionItem[]>(() => {
    ...
    return pool
      ...
      .slice(0, 8)
      .map((row) => row.it)
  }, [trigger, assetItems, folderItems, actionPageItems])
  ...
  const evaluateTrigger = useCallback(() => {
    ...
    while (i >= 0) {
      ...
      if (ch === '@' || ch === '#' || ch === '!') {
        ...
        // Prevent re-triggering inside an already-inserted !actionpage:slug token
        if (ch === '!' && query.includes(':')) {
          setTrigger(null)
          return
        }
    ...
    (item: MediaMentionItem) => {
      ...
      const token =
        item.kind === 'action_page'
          ? `!actionpage:${item.slug} `
          : `${item.kind === 'asset' ? '@' : '#'}${item.slug} `
      const next =
  ...
  const openPicker = useCallback(
    (char: '@' | '!' | '#') => {
      ...
      const coords = measureCaret(pos)
      setTrigger({ char, start: pos, query: '', coords })
    },
    [value, measureCaret],
  )
  ...
  return (
    <div className="mention-ta-outer">
      <div className="mention-ta-toolbar">
        <button
          type="button"
          className="mention-ta-toolbar-btn"
        ...
        <button
          ...
          onMouseDown={(e) => e.preventDefault()}
          onClick={() => openPicker('!')}
          title="Insert action page mention"
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
...
function renderHighlighted(text: string): React.ReactNode[] {
  ...
  const re = /(!actionpage:[a-z0-9][a-z0-9_-]*|[@#][a-z0-9][a-z0-9_-]*)/gi
  let last = 0
  ...
  while ((m = re.exec(text))) {
    ...
    const tok = m[0]
    const cls = tok.startsWith('!actionpage:')
      ? 'media-mention media-mention--action-page'
      : tok.startsWith('#')
