// src/app/api/cron/lead-sequences/route.ts
//
// Every-minute tick: find lead_sequence_runs whose next_run_at is past due with
// no job queued, then enqueue one messenger_jobs row per run. The messenger
// worker drains them (per-thread serialized) via the `lead_sequence_send` kind.
// Mirrors cron/project-sequences.
...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  if (error) {
    console.error('[cron.lead-sequences] query failed', error.message)
  ...
  for (const r of rows) {
    ...
    if (jobErr || !job) {
      console.warn('[cron.lead-sequences] enqueue failed', r.id, jobErr?.message)
      continue
    }
      ...
      try {
        after(async () => {
          try {
            ...
            console.warn('[cron.lead-sequences] worker trigger failed', e)
          }
        })
      } catch {
