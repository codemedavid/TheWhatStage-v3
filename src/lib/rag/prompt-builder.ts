...
function assembleSystemPrompt(p: ChatbotPersona, contextBlock: string, conversationSummary?: string, paymentEnumBlock?: string): string {
  ...
  const stable = [
    ...
    `- Product/property details (price, specs, availability, location, room count, lot size, amenities): only state what is in the [product:…] or context chunks. Never estimate, round, or fill in missing fields.`,
    `- Payment methods: only mention methods listed in the payment enum block above. Never invent account numbers, QR codes, bank names, or instructions not shown there. If no payment method is listed, say you will check with the owner.`,
    `- Sales page links and booking URLs: never construct, guess, or paraphrase a URL. Only send the exact deeplink token the system provides in your Instructions — never fabricate a URL.`,
    `- Customer orders and booking status: you have no access to order history or booking records. If a customer asks about their order, appointment, or booking status, use the Fallback message and offer to escalate to the owner.`,
    `- When the context contains multiple products or properties, never mix attributes across items (e.g., applying property A's price to property B's description). Each [product: id] block is a separate item.`,
    ``,
    `# Fallback`,
